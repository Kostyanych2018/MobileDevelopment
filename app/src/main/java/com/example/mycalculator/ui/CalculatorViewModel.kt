...
class CalculatorViewModel : ViewModel() {
    ...
    private fun enterNumber(number: Int) {
        ...
        state =
            state.copy(number2 = updated, displayText = updated.ifEmpty { "0" }, isError = false)
    }
    ...
    private fun performCalculation() {
        ...
        val expression = "${state.number1} ${operation.symbol} ${state.number2}"
        ...
        historyRepository.saveHistory(
            ...
            onSuccess = {
                Log.d("HistoryDebug", "Saved: $expression = $text")
                historyRepository.enforceLimit(20)
            },
            onError = { error ->
                Log.e("HistoryDebug", "Save failed", error)
            }
        )
